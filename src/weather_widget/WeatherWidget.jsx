...
export default function WeatherWidget(props) {
	...
	useEffect(() => {
		axios
			...
			.then((resp) => {
				setCurrentData({
					temperature: `${Math.round(parseFloat(resp.data.current.feels_like))}`,
					weatherCode: `${resp.data.current.weather[0].id.toString()}`});
				
				setCurrentTemp(`${Math.round(parseFloat(resp.data.current.feels_like))}`);
	...
	const weatherIcon = (weatherCode) => {
		...
		switch (weatherCode.substr(0,1)) {
			case "2":
				return <img src={IconStorm} className="weather-widget__icon" alt="thunderstorm" />;
			case "3":
				return <img src={IconRain} className="weather-widget__icon" alt="rain" />;
			case "5":
				return <img src={IconRain} className="weather-widget__icon" alt="rain" />;
			case "6":
				return <img src={IconSnow} className="weather-widget__icon" alt="snow" />;
			case "8":
				return <img src={IconCloudy} className="weather-widget__icon" alt="sunny" />;
			default:
				return null;
		}
	}
